...
# Define function to find and process squad CSV
def process_squad_csv(input_folder="data", output_filename="data/SquadPlayerNames.csv"):
    # Find the squad CSV file
    match_filename = None
    match_number = None
    for filename in os.listdir(input_folder):
        if filename.startswith("SquadPlayerNames_IndianT20League - Match_") and filename.endswith(".csv"):
            try:
                # Extract match number from filename using regex
                match = re.search(r'Match_(\d+)\.csv$', filename)
                if match:
                    match_number = int(match.group(1))
                    match_filename = os.path.join(input_folder, filename)
                    break
            except ValueError:
                continue

    if not match_filename or match_number is None:
        raise FileNotFoundError(f"No valid squad CSV found in folder '{input_folder}'.")

    # Copy file to new name
    shutil.copyfile(match_filename, output_filename)
    print(f"Copied '{match_filename}' to '{output_filename}'")
    return match_number
...
if __name__ == "__main__":
    try:
        # Automatically process squad CSV and get match number
        match_number = process_squad_csv()
        print(f"Processing Match {match_number}")
        
        # Load squad dataframe after processing
        ...
        # Prepare player selection dataframe
