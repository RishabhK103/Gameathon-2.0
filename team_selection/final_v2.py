...
# Normalize ground names to avoid whitespace issues
ground_mapping_df["Ground"] = ground_mapping_df["Ground"].apply(lambda x: re.sub(r'\s+', ' ', str(x)).strip())
ground_df["Ground"] = ground_df["Ground"].apply(lambda x: re.sub(r'\s+', ' ', str(x)).strip())
...
def optimize(team1, team2):
    ...
    team_df["Adjusted_Score"] = team_df["Score"]
    ...
    prob += pulp.lpSum([x[i] for i in players]) == total_players
    prob += pulp.lpSum([x[i] for i in batters.index]) >= 1
    prob += pulp.lpSum([x[i] for i in bowlers.index]) >= 1
    prob += pulp.lpSum([x[i] for i in bowling_options.index]) >= 5
    prob += pulp.lpSum([x[i] for i in keepers.index]) >= 1
...
if __name__ == "__main__":
    try:
        match_number = int(input("Enter Match Number (1–74): "))
        selected_ground = ground_mapping_df.iloc[match_number - 1]["Ground"]
    except:
        print("Invalid Match Number!")
        exit(1)

    if selected_ground not in ground_df["Ground"].values:
        print(f"Ground '{selected_ground}' not found in ground data.")
        print("Available grounds:", list(ground_df["Ground"]))
        exit(1)

    ground_data = ground_df[ground_df["Ground"] == selected_ground].iloc[0]
    batter_weight = float(ground_data['Batting'])
    keeper_weight = batter_weight
    ...
    if best_team is not None:
        ...
        print("No valid team could be formed.")
